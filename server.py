...
def fullCalculator():
    if request.method == 'POST':
        ...
        if os.path.exists('./resultsBin/fullCalcListCam.pickle'):
            ...
            fullCalcDict = {}
        res=make_response(render_template('full_calculator.html',
                               length=len(fullCalcDict),
                               links=fullCalcDict))
        res.status='OK'
        res.status_code=200
        loger.shortResponseMessage(res)
        loger.shortRequestMessage(request)
        loger.responseMessage(res)
        loger.requestMessage(request)
        return res
