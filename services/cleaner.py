...
def envCleaner(value: str):
    ...
    """
    if value.lower() == 'bin':
        path = Path(os.environ.get('PROJECT_PATH')) / Path('resultsBin')

    elif value.lower() == 'plot':
        path = Path(os.environ.get('PROJECT_PATH')) / Path('templates/plots')

    elif value.lower() == 'table':
        path = Path(os.environ.get('PROJECT_PATH')) / Path('templates/tables')
